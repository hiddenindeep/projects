...
if db_type == "sqlite":
    ...
    print(f"{db_type}://{username}:{password}@{host}:{port}/{database}")
...
#     title = Column(String(255))  # 指定VARCHAR长度
#     category = Column(String(100))  # 指定VARCHAR长度
...
#     file_path = Column(String(500))  # 指定VARCHAR长度
#     file_type = Column(String(50))  # 指定VARCHAR长度
#     create_dt = Column(DateTime, default=datetime.utcnow)
#     update_dt = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
