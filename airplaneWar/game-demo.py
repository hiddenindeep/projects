...
class Enemy():
	def __init__(self):
		# 添加敌人
		self.img = pygame.image.load('./airplaneWar/enemy.png')
...
class Bullet():
	def __init__(self):
		# 画出子弹
		self.img = pygame.image.load('./airplaneWar/bullet.png')
...
# 引入UFO游戏左上角的游戏图标图片
icon = pygame.image.load('./airplaneWar/ufo.png')	#外部加载图像
pygame.display.set_icon(icon)						#设置左上角的游戏图标，图标尺寸大小为32*32
# 背景图片
bgImg = pygame.image.load('./airplaneWar/bg.png')

#引入飞机图片
playerImg = pygame.image.load('./airplaneWar/player.png')
playerX = 400 	#玩家的X坐标
playerY = 500 	#玩家的Y坐标
