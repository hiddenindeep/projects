...
def create_chat_agent(instructions: str, external_client: AsyncOpenAI) -> Agent:
    """闲聊 Agent：不使用 MCP 工具，只负责自然对话。"""
    return Agent(
        name="ChatAgent",
...
def create_stock_agent(
    ...
    """股票 Agent：挂载股票相关 MCP 工具，专注于股票分析与数据查询。"""
    return Agent(
        name="StockAgent",
        instructions=instructions,
...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    """
    统一对话入口：
    - 闲聊 Agent：自然闲聊、不调用股票工具。
    - 股票 Agent：当 task 为“股票分析”或显式传入工具时，调用股票相关 MCP 工具。
    通过共享 session_id，在同一会话中可以在两个 Agent 之间进行 handoff。
    """
    # 对话管理，通过session id
    if session_id:
        with SessionLocal() as session:
    ...
    # 根据 task / tools 选择使用哪个 Agent
    # - 股票分析 或 显式传入工具列表 -> 股票 Agent
    # - 其他情况 -> 闲聊 Agent
    use_stock_agent = (task == "股票分析") or (tools and len(tools) > 0)
    ...
    # 闲聊 Agent：默认不使用工具，直接对话
    if not use_stock_agent:
        agent = create_chat_agent(instructions=instructions, external_client=external_client)
