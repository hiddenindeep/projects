...
if user_input:
    try:
        st.session_state['messages'] = [{'role': 'user', 'content': f'{user_input}'}]

        client = openai.OpenAI(
            api_key=os.environ["OPENAI_API_KEY"],  # https://bailian.console.aliyun.com/?tab=model#/api-key
        ...
        if response_message.function_call:
            ...
            if function_name == 'plotStockPrice':
                ...
                st.session_state['messages'].append({'role': 'assisant', 'content': second_response.choices[0].message.content})
        else: 
